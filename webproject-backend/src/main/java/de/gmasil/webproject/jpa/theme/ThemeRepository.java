...
public interface ThemeRepository extends JpaRepository<Theme, Long> {
    ...
    @Query("SELECT t FROM THEME t, PROPERTY p WHERE p.key = '" + Property.DEFAULT_THEME + "' AND t.id = p.value")
    public Optional<ThemeProjectionNEW> findDefaultProjection();

    @Query("SELECT t FROM THEME t, USER u WHERE t = u.activeTheme AND u.id = :userId")
    public Optional<ThemeProjectionNEW> findProjectionActiveByUser(@Param("userId") Long userId);
    ...
    public List<ThemeProjectionNEW> findAllProjectionByPresetTrue();

    public List<ThemeProjectionNEW> findAllProjectionByPresetTrueOrCreator(User creator);
}
